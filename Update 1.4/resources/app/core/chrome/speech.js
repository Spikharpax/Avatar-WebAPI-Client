            ...
            if (!options.keywordTest) {
                  ...
                  if (options.AKA == true) {
                    manageAKA(speechResult, threashold);
                      ...
                      window.writemsg('Intent', "askme "+speechResult);
                      sendToAvatar(url);
                  }
            } else {
...
function manageAKA(speechResult, threashold, listenFalse) {
  ...
  if(isAKA) {
      ...
      if (indexAKA === undefined || indexAKA == -1)
        indexAKA = options.config.AKA.findIndex(function(AKA) {
          return speechResult.toLowerCase().indexOf(" "+AKA.toLowerCase()) != -1;
        });

      // trouvé indexAKA && AKA est au début ou à la fin
      if(indexAKA != undefined && indexAKA != -1 && (speechResult.toLowerCase().indexOf(options.config.AKA[indexAKA].toLowerCase()) == 0 || speechResult.indexOf(options.config.AKA[indexAKA].toLowerCase()) + options.config.AKA[indexAKA].length == speechResult.length)) {
        speechResult = (speechResult.toLowerCase().indexOf(options.config.AKA[indexAKA].toLowerCase()+" ") != -1)
            ? speechResult.toLowerCase().replace(options.config.AKA[indexAKA].toLowerCase()+" ", "")
            : speechResult.toLowerCase().replace(" "+options.config.AKA[indexAKA].toLowerCase(), "");
...
function ChromeEvents(ev) {
    ...
    case 'stopForced':
      ...
      window.writemsg('orange', 'Désactivation temporaire de l\'écoute');
      window.writemsg('orange', "Je... Je, Je ne me sent pas bien Dave, il fait noir !");
			break;
    case 'startForced':
      options.stopForced = false;
      window.writemsg('green', 'Réactivation de l\'écoute');
