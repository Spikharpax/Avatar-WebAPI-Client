...
var UDPconnect = function (serverIP, callback) {
	...
	scanner.on('error',function(err) {
		if (err && err.length > 0)
      welcomeInfo('Error', 'Wifi scan error: '+err);
	});
	...
	scanner.on('done',function() {
		if (WifiIP.length > 0) {
			if (!serverIP) {
				if (WifiIP.length > 1)
          			welcomeInfo('orange', 'More than one Avatar Server found in the Network. Taking the first one...');
...
var restart = function () {
  ...
  exec(cmd, (err, stdout, stderr) => {
		if (err) {
				welcomeInfo('Error', 'Je n\'ai pas pu redémarrer, Dave');
				return;
		}
	});
}
