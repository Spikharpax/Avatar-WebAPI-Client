...
var init = exports.init = function () {
    ...
    Chrome.Navigator(() => {
      Chrome.HTTPSServer(setup, (ee) => {
          eeToChrome = ee;
          Avatar.SpeakManager.setOptions(eeToChrome, Config, socket);
          record_Chrome();
      });
    });
}
  ...
  .then((voices) => {
    if (voices && voices.length > 0) {
      if (withAKA == 2) callback(voices);
        ...
        if (withAKA == 2) callback();
        else logger('orange', 'Aucune voix disponibles, Dave.');
    }
  }).catch((err) => {
    if (withAKA == 2) callback();
    else logger('Error', "Je n'ai pas pu récupérer les voix systèmes, Dave");
  });
...
var intercom = exports.intercom = function(to) {
  eeToChrome.emit ('intercom', to);
}


function record_Chrome (callback) {

	let eeFromChrome = new EventEmitter();

  // Set event ID to Chrome
	eeToChrome.emit ('id', eeFromChrome);

  // get Voices for properties
  eeFromChrome.on('getVoices', () => {
    getVoices(2, (voices) => {
      eeToChrome.emit('setVoices', voices);
    });
  });

  eeFromChrome.on('testVoice', (speech, voice, speed, volume) => {
      testVoice(speech, voice, speed, volume);
  });

  // Config
  eeFromChrome.on('Config', (params) => {
    if (params) {
      Config = params;
  ...
  // AKA only
	eeFromChrome.on('AKA', (threashold) => {
      AKA();
	});

  // AKA with action
  eeFromChrome.on('Action', (buffer, threashold) => {
  ...
  // action only
  eeFromChrome.on('Listen', (buffer, threashold) => {
  ...
  // Intercom
  eeFromChrome.on('Intercom', (to) => {
      sending_intercom(to);
  });

  eeFromChrome.on('Close', (flag) => {
      if (flag == "true") {
      ...
      remote.dialog.showMessageBox(win, options, function(response) {
          ...
          if (response == 0) {
             ...
             eeToChrome.emit('quit', false);
      });
  });

  // Aide
  eeFromChrome.on('help', () => {
...
var logger = exports.logger = function (type, msg) {
    if (eeToChrome) eeToChrome.emit('log', type, msg);
}
...
function sending_intercom (to) {
  ...
  if (Config.speech.server_speak && to.toLowerCase() == Config.client.toLowerCase()) {
    ...
    let intercomPlayer = new FFplay(file, null, function() {
      intercomPlayer = null;
      end(true);
    });
    return;
...
function stopAKA() {
  return new Promise(function (resolve, reject) {
    eeToChrome.emit ('stopAKA', () => { // Stop le AKA && restitution de l'écoute Intent, au cas ou... intercom par exemple
        resolve();
    });
  });
}
...
var OnOffListen  = exports.OnOffListen = function(on) {
  if (on)
    //eeToChrome.emit ('start', () => {
      eeToChrome.emit ('startAKA');
    //});
  else
    eeToChrome.emit ('stop');
}
...
function reset_listen(full) {
    ...
    eeToChrome.emit ('startAKA');
		// Reinit variables
		is_restart = is_askme.active = is_listen = is_silence = false;
    is_askme.options = null;
	}
...
var askme = exports.askme = function(options){
	...
	if (!options.tts) {
    logger('orange', 'Aucune réponse. J\'interromps le dialogue.');
		return;
  }
